...
std::vector<int> selectObjectToGrasp(std::vector<PointCloudT::Ptr > candidates){
	//currently, we just pick the one with the most points
	std::vector<int> objects;
	int max_num_points = -1;
	int max2_num_points = -1;
	int index = -1;
	int index2 = -1;
	for (unsigned int i = 0; i < candidates.size(); i ++){
		if ((int)candidates.at(i)->points.size() > max_num_points){
			...
			index2 = index;
			index = (int)i;
		else if((int)candidates.at(i)_.points.size() > max2_num_points)
			max2_num_points = (int)candidates.at(i)->points.size();
			index2 = (int)i;
		}
	}
	objects.push_back(index);
	objects.push_back(index2);
	return objects;
}
...
int main(int argc, char **argv) {
	...
	int selected_object = selectObjectToGrasp(detected_objects).at(0);
