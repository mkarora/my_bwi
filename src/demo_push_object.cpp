...
geometry_msgs::PoseStamped pclToPoseStamped(sensor_msgs::PointCloud2 pc2){
	...
	//create a pose on the right side of point cloud
	pcl::PointXYZ min;
	pcl::PointXYZ max;
	...
	pose_1.position.x=centroid(0);
	pose_1.position.y=min.y - 0.05;
	pose_1.position.z=centroid(2) - 0.02;
	pose_1.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,3.14/2,0);

	geometry_msgs::PoseStamped stampedPose;

	stampedPose.header.frame_id = pc2.header.frame_id;
	stampedPose.header.stamp = ros::Time(0);
	stampedPose.pose = pose_1;
...
int main(int argc, char**argv){
	...
	ros::init(argc, argv, "demo_push_object");
	ros::NodeHandle n;
	...
	ros::Publisher pose_pub = n.advertise<geometry_msgs::PoseStamped>("/demo_push_object/pose", 10);;
